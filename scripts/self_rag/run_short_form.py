...
import sys
import time
from self_rag.utils import PROMPT_DICT, TASK_INST, load_jsonlines, control_tokens, load_special_tokens
from self_rag.metrics import match, accuracy
from generate_score import ConstantConfig
...
def call_model_rerank_w_scores_batch(prompt, query_token:str, evidences, model, max_new_tokens=15,
    ...
    if do_retrieve is True:
        evidence_augmented_inputs = [prompt + f"[Retrieval]{query_token}" + "<paragraph>{0}\n{1}</paragraph>".format(
    ...
    # Aggregating answers
    if len(results) == 1 and len(evidences) != 1:
...
def process_data_evidences(demonstration, top_n):
    ...
    my_score = False
    my_evidence = []
    for item in demonstration[ctx_key]:
        if 'my_score' in item:
            my_score = True
        # if 'score' in item:
        #     my_evidence.append(item)
        my_evidence.append(item)
    if my_score:    # 'my_score' in demonstration[ctx_key][0]
        # print("mike sort")
        my_demo = sorted(my_evidence, key=lambda x: x['my_score'], reverse=True)[:top_n]
        return prompt, my_demo
    else:
        print("default sort")
...
def main():
    ...
    def generate(prompt, evidences, max_new_tokens):
        return call_model_rerank_w_scores_batch(prompt, 
                                                query_token=ConstantConfig.get_question_token_for_repr(),
                                                evidences=evidences, model=model, max_new_tokens=max_new_tokens,
